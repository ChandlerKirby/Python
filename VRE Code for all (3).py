...
def gettingCSVgroupName(files):
    groupOfCSVfiles = []
    for file in files:
        # groupName = file.split('CHANGE_')
        groupName = file.split('_RefYear')[0]
        groupName = groupName.split('.')[0]
        if groupName not in groupOfCSVfiles:
            groupOfCSVfiles.append(groupName)
    return groupOfCSVfiles
gettingCSVgroupName(files)




def getonefinalresult(a):
    NAME1=a
    NAME2=''
    NAME3=''
...
a=gettingCSVgroupName(files)[0]
i=0
for i in range(len(gettingCSVgroupName(files))):
    a=gettingCSVgroupName(files)[i]
    getonefinalresult(a)
